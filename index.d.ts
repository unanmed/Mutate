declare module 'mutate-game' {
    ...
    export interface LoadEvent<K extends keyof CoreEventMap>
        extends MutateEvent<K, CoreEventMap> {
        /** 加载时长 */
        time: number;
    }

    export interface StartEvent<K extends keyof CoreEventMap>
        extends MutateEvent<K, CoreEventMap> {
        music: AudioBuffer;
        mtt: MutateChart;
    }

    export interface TriggerEvent<K extends keyof CoreEventMap>
        extends MutateEvent<K, CoreEventMap> {
    ...
    export interface HitEvent<K extends keyof NoteEventMap>
        extends MutateEvent<K, NoteEventMap> {
        res: JudgeRes;
        note: BaseNote<NoteType>;
        base: Base;
    ...
    export interface HoldEvent<K extends keyof NoteEventMap>
        extends HitEvent<K> {
        time: number;
        totalTime: number;
    }
    ...
    export interface RenderEvent<K extends keyof RenderEventMap>
        extends MutateEvent<K, RenderEventMap> {
        ctx: CanvasRenderingContext2D;
        canvas: HTMLCanvasElement;
    }

    export interface EffectEvent<K extends keyof RenderEventMap>
        extends RenderEvent<K> {
        effect: ToDrawEffect;
    }
    ...
    class Mutate extends MutateEventTarget<CoreEventMap> {
         ...
         */
        load(music: string, mtt: string, onMusicProgress?: (e: ProgressEvent) => void, onMTTProgress?: (e: ProgressEvent) => void): Promise<void>;
    ...
    class Renderer extends MutateEventTarget<RenderEventMap> {
         ...
         */
        setNote<T extends NoteType>(
            type: T,
            fn: (e: BaseNote<T>) => void
        ): void;
