...
export class MutateEventTarget<T extends MutateEventMap<keyof T & string>> {
    ...
    /** 所有的事件 */
    private readonly events: MutateEventList<T>

    constructor(init: MutateEventList<T>) {
        this.events = init;
    }
    ...
    off<K extends keyof T>(type: K, listener: EventFn<T[K]>): void {
        ...
        if (i === -1) throw new ReferenceError(`You are trying to remove a nonexistent listener.`);
